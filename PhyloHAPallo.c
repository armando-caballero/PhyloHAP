...
/* PhyloHAPallo.c (Littorina allopatric model; n haploid subpopulations RB and SU;
1 selected locus s=0.99; 1 tag original haplotype; and x neutral; study only extreme ones) */

# include "libhdr"
# include <time.h>
# include <string.h>

# define NmaxS 25 // max pop
# define NmaxI 20001 // max nind and haplotypes
...
int gen, GEN, pregen, PREGEN, TREEGEN, l1, l2;
...
main()
{
	...
	for (pregen=1; pregen<=PREGEN; pregen++)
	{
		// THERE IS NO MIGRATION BETWEEN ANY POPULATION
		// IN THE LAST GENERATION ALL INDIVIDUALS COME FROM POPs 1 AND 2
		mutation();
		fitnesses();
		if (pregen==PREGEN)	mating_allopatric();
		else			mating();
	}
	output_file_gen0();
...
getinputs()
{
	...
	getintandskip("Number of subpopulations (max 100):",&pop,1,22);
	getintandskip("Number of individuals (max 10000) for subpopulation :",&nind,1,20000);
	NIND = nind * pop;
	...
	getrealandskip("Length of sequence in Morgans (99:FreeRecom) :",&L,0.0,99.0);
	getrealandskip("Selection coefficient:",&s,0.0,1.0);
	getintandskip("Number of pregenerations:",&PREGEN,1,infinity);
	getintandskip("Number of generations:",&GEN,1,infinity);
	getintandskip("Number of generations between trees:",&TREEGEN,1,infinity);
	getintandskip("Number of sampled individuals:",&NSAM,1,100);
...
headings()
{
	...
	printf(" u=%10.9f    L=%f    mECO=%f    mLOC=%f    \n", u, L, mrECO, mrLOC);
}
...
mating_allopatric()
{
	double rnd, cum, cum_pm_s[NmaxI];

	// IN THE LAST PREGENERATION ALL INDIVIDUALS COME FROM POPs 1 (RB) and 2 (SU)
	...
	for (i=1; i<=pop; i++)
	{
		// ONLY PARENTS FROM THE FIRST POPULATIONS (1 and 2)
		...
		if (i%2 != 0)
		{
			...
			{
				fprintf(fsal,"\ncum = %f\n", cum);
				fprintf(fsal,"\n Scaled relative fitnesses subpop=1\n");
				for (l=1; l<=nind; l++)   fprintf(fsal,"cum_pm_s = %f\n", cum_pm_s[l]);
			}
			if ((tracelevel!=0)&&(gen == GEN))	fprintf(fsal,"\n Parents of subpopulation 1\n");
			...
			for (l=1; l<=nind; l++)
			{
				...
				if (uniform() < 0.5)		for (k=0; k<NCRO; k++)	gm[i][l][k]=((EE[k]&sm[1][p1][k])|(FF[k]&sm[1][p2][k]));
				else				for (k=0; k<NCRO; k++)	gm[i][l][k]=((FF[k]&sm[1][p1][k])|(EE[k]&sm[1][p2][k]));
			}
		}
		else
		{
			...
			{
				fprintf(fsal,"\ncum = %f\n", cum);
				fprintf(fsal,"\n Scaled relative fitnesses subpop=2\n");
				for (l=1; l<=nind; l++)   fprintf(fsal,"cum_pm_s = %f\n", cum_pm_s[l]);
			}
			if ((tracelevel!=0)&&(gen == GEN))	fprintf(fsal,"\n Parents of subpopulation 2\n");
			...
			for (l=1; l<=nind; l++)
			{
				...
				if (uniform() < 0.5)		for (k=0; k<NCRO; k++)	gm[i][l][k]=((EE[k]&sm[2][p1][k])|(FF[k]&sm[2][p2][k]));
				else				for (k=0; k<NCRO; k++)	gm[i][l][k]=((FF[k]&sm[2][p1][k])|(EE[k]&sm[2][p2][k]));
			}
		}
	}
...
writing_phylfile()
{
	...
	strcat(outname,"phylfreqfile"); // reinicializa el array con el nombre base, no haría falta si se hubiera limpiada solo la parte que cambia
